    ...
    <div class="p-6 flex-1 overflow-y-auto space-y-5 bg-gray-50/50" id="chat-container">
        @forelse($comments as $comment)
            <div class="flex {{ $comment->user_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-[85%] sm:max-w-[75%] rounded-2xl px-5 py-3 shadow-sm {{ $comment->user_id === auth()->id() ? 'bg-indigo-600 text-white rounded-br-sm' : 'bg-white border border-gray-100 text-gray-800 rounded-bl-sm' }}">
                    <div class="text-[11px] {{ $comment->user_id === auth()->id() ? 'text-indigo-200' : 'text-gray-400' }} mb-1 font-semibold tracking-wide uppercase">
    ...
    <div class="p-4 sm:p-5 bg-white border-t border-gray-100 shrink-0">
        <form wire:submit="submitComment">
            <div class="flex items-end gap-3 relative">
                <div class="flex-1">
                    <textarea wire:model="body" rows="2" class="block w-full rounded-xl border-gray-200 bg-gray-50 shadow-sm focus:bg-white focus:border-indigo-500 focus:ring-indigo-500 text-sm resize-none p-3.5 transition-colors placeholder-gray-400" placeholder="Ketik pesan Anda di sini..." required></textarea>
                </div>
                <button type="submit" class="bg-indigo-600 text-white w-12 h-12 rounded-xl hover:bg-indigo-700 hover:shadow-md hover:shadow-indigo-500/20 transition-all flex items-center justify-center shrink-0 disabled:opacity-50 disabled:cursor-not-allowed" wire:loading.attr="disabled">
