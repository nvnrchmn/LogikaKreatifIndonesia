        ...
        <div class="container-narrow">
            ...
            <div class="text-center max-w-3xl mx-auto mb-12">
                ...
                <p class="text-txt-muted text-sm leading-relaxed mb-4">
                    Pilih paket layanan digital yang sesuai dengan kebutuhan bisnis Anda. Pembayaran aman dan terverifikasi secara otomatis via <strong>Xendit Payment Gateway (Mode Tes & Live IDR)</strong>.
                </p>
                <div class="inline-flex items-center gap-2 bg-brand-primary/10 text-brand-primary px-4 py-1.5 rounded-full text-xs font-semibold">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-16">
                @forelse($packages as $package)
                    <div class="relative card-hover p-8 flex flex-col {{ $package->is_featured ? 'border-2 border-brand-primary shadow-lg' : '' }}">
                        ...
                        <div class="mb-6">
                            ...
                            @endif
                            <div class="font-display text-3xl font-bold text-brand-primary">{{ $package->formatted_price }}</div>
                        </div>
                ...
                @empty
                    <p class="text-center text-txt-muted col-span-3 py-8">Paket layanan belum dimuat. Silakan jalankan db:seed.</p>
                @endforelse
            </div>

            <!-- Single Dedicated Checkout Form (Always visible for easy Xendit review) -->
            <div x-ref="checkout-section" class="max-w-2xl mx-auto bg-white rounded-3xl p-8 md:p-12 border border-border-minimal shadow-md mb-16">
                <div class="flex items-center gap-3 mb-6 pb-6 border-b border-border-minimal">
                    ...
                    <div>
                        <h3 class="font-display font-bold text-2xl text-txt-main">Formulir Checkout Produk & Pembayaran</h3>
                        <p class="text-xs text-txt-muted">Pilih paket di bawah ini untuk memproses tagihan resmi via Xendit.</p>
                    </div>
                </div>
                ...
                @if($packages->count() > 0)
                    ...
                    @foreach($packages as $package)
                        <div x-show="selected === {{ $package->id }}" class="space-y-6">
                            <div class="p-4 bg-canvas-light rounded-2xl border border-border-minimal flex items-center justify-between">
                                <div>
                                    <span class="text-xs font-semibold text-brand-primary uppercase tracking-wider">Paket Dipilih</span>
                                    <h4 class="font-bold text-txt-main text-lg">{{ $package->name }}</h4>
                                </div>
                                <div class="text-right">
                                    <span class="text-xs text-txt-muted block">Harga Paket</span>
                                    <span class="font-display font-bold text-xl text-txt-main">{{ $package->formatted_price }}</span>
                                </div>
                            </div>
                            ...
                            <form action="{{ route('packages.checkout', $package->slug) }}" method="POST" class="space-y-5">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-xs font-semibold uppercase tracking-wider text-txt-main mb-2">Nama Lengkap / Pelanggan *</label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold uppercase tracking-wider text-txt-main mb-2">Alamat Email Pembayar *</label>
                                ...
                                <div>
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-txt-main mb-2">Nomor Telepon / WhatsApp *</label>
                                    <input type="text" name="guest_phone" value="{{ old('guest_phone') }}" required
                                        class="w-full px-4 py-3 bg-canvas-light border border-border-minimal rounded-xl text-sm focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-all text-txt-main"
                                        placeholder="081234567890">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold uppercase tracking-wider text-txt-main mb-2">Catatan / Deskripsi Pesanan (Opsional)</label>
                                ...
                                <div class="p-4 bg-brand-primary/5 rounded-xl border border-brand-primary/10 text-xs text-txt-muted flex items-center gap-3">
                                    <svg class="w-6 h-6 text-brand-primary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                                    <span>Klik di bawah untuk memuat invoice Xendit. Anda akan diajukan ke gateway pembayaran resmi Xendit.</span>
                                </div>

                                <button type="submit" class="w-full bg-brand-primary text-white font-bold py-4 rounded-xl hover:bg-brand-primary/90 transition-colors shadow-lg shadow-brand-primary/30 flex items-center justify-center gap-2">
            ...
            <div class="max-w-2xl mx-auto bg-canvas-dark rounded-3xl p-8 text-white shadow-lg border border-brand-primary/30">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
                    <div>
                        ...
                        <strong class="text-white text-base">PT. Logika Kreatif Indonesia</strong>
                    </div>
                    <div>
