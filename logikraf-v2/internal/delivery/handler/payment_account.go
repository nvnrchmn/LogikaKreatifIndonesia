...
func GetReconciliation(c fiber.Ctx) error {
	...
	if err := model.DB.Where("tenant_id = ?", tenant).Find(&txs).Error; err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "failed"})
	}
	var gross, providerFee, platformFee, net uint64
	byStatus := map[string]uint{}
	for _, t := range txs {
		gross += uint64(t.GrossAmount)
		providerFee += uint64(t.ProviderFee)
		platformFee += uint64(t.PlatformFee)
		net += uint64(t.NetAmount)
		byStatus[t.Status]++
	}
...
// Read-only: Logikraf only records entitlement, never holds funds.
func ListPaymentTransactions(c fiber.Ctx) error {
	tenant := c.Locals("tenant").(string)
	var txs []model.PaymentTransaction
	if err := model.DB.Where("tenant_id = ?", tenant).Order("created_at desc").Limit(200).Find(&txs).Error; err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "failed"})
	}
	return c.JSON(txs)
}
...
func ListTenantPaymentAccounts(c fiber.Ctx) error {
	tenant := c.Locals("tenant").(string)
	var accounts []model.TenantPaymentAccount
	if err := model.DB.Where("tenant_id = ?", tenant).Order("provider").Find(&accounts).Error; err != nil {
		return c.Status(500).JSON(fiber.Map{"error": "failed"})
	}
