...
function FolderIcon() { return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" /></svg> }
...
export default function AdminLayout() {
  ...
  const activeNav = navItems.find(i => location.pathname === i.path || location.pathname.startsWith(i.path + '/'))
  ...
  return (
    <div className="min-h-screen bg-canvas-overlay">
      ...
      {/* Sidebar desktop */}
      <aside className={`hidden lg:flex flex-col bg-canvas-dark border-r border-white/10 transition-all duration-200 ${sidebarOpen ? 'w-64' : 'w-16'}`}>
        <div className="h-16 flex items-center gap-3 px-4 border-b border-white/10">
          <img src="/logo.png" alt="Logikraf" className="h-9 w-auto object-contain shrink-0" />
          {sidebarOpen && <span className="text-white font-display font-extrabold text-lg tracking-tight">LOGIKRAF</span>}
        </div>
          ...
          <div className="flex items-center gap-2 px-2 py-2">
            ...
            {sidebarOpen && (
              <div className="overflow-hidden">
                ...
                <div className="text-text-light/50 text-xs">admin@logikraf.id</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* Sidebar mobile */}
      {mobileOpen && (
        <div className="fixed inset-y-0 left-0 z-50 w-64 bg-canvas-dark border-r border-white/10 lg:hidden">
          <div className="h-16 flex items-center gap-3 px-4 border-b border-white/10">
          <img src="/logo.png" alt="Logikraf" className="h-9 w-auto object-contain shrink-0" />
            <span className="text-white font-display font-extrabold text-lg tracking-tight">LOGIKRAF</span>
          </div>
